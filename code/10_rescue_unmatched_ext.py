...
# ============================================================
unm = pd.read_csv('data/processed_extended/ceo_speech_unmatched.csv',
                  dtype={'gvkey': str})
...
existing = pd.read_csv('data/processed_extended/ceo_speech_manifest.csv',
                       dtype={'gvkey': str})
combined = pd.concat([existing, pd.DataFrame(recovered)], ignore_index=True)
combined.to_csv('data/processed_extended/ceo_speech_manifest.csv', index=False)
print(f"\nManifest now contains {len(combined)} total extractions")

pd.DataFrame(still_unmatched).to_csv(
    'data/processed_extended/ceo_speech_still_unmatched.csv', index=False
)
print(f"Still-unmatched log: data/processed_extended/ceo_speech_still_unmatched.csv")
